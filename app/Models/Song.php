...
class Song extends Model
{
    ...
    public function getLength()
    {
        ...
        return Song::getDuration($relativePath . "Song" . $this->getAttribute('id'));
    }
    public function getSize()
    {
        $relativePath = "public\\Files\\";
        return filesize($relativePath . "Song" . $this->getAttribute('id'));
    }
    ...
    public static function getDuration($filename, $use_cbr_estimate=false)
    {
        $fd = fopen($filename, "rb");

        $duration=0;
        $block = fread($fd, 100);
        $offset = Song::skipID3v2Tag($block);
        ...
        while (!feof($fd))
        {
            ...
            {
                return Song::estimateDuration($filename, $info['Bitrate'],$offset);
            }
        }
        return round($duration);
    }

    private static function estimateDuration($filename, $bitrate,$offset)
    {
        $kbps = ($bitrate*1000)/8;
    ...
    private static function skipID3v2Tag(&$block)
    {
